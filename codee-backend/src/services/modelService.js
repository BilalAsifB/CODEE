...
export const callHuggingFaceAPI = async (inputs, params = {}, modelId) => {
  ...
  try {
    ...
    if (error.response?.status === 429) {
      throw new Error(
        "Rate limited by Hugging Face API. Please try again later."
      );
    }
    if (error.response?.status === 503) {
      throw new Error(
        "Model is loading. Please wait and try again in 20-30 seconds."
      );
    }
    if (error.code === "ECONNABORTED") {
      throw new Error(
        "Request timeout. Model may be loading. Please try again."
      );
    }
